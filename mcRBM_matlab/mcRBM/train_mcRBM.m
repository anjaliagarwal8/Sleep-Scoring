...
%%
% W = Weight matrix for mean part
% vb = visible bias
% hb_mean = mean bias
% hb_cov = covariance bias
% FH = Factor Hidden Matrix 
% VF = Visible Factor Matric

% -----------------Mean RBM------------------------------
% Visible Units --(W,hb_mean)-> Mean Hidden Units 

% -----------------Covariance RBM-------------------------
% Visible Units --(VF)--> Covariance features --(FH,hb_cov)--> Covariance Hidden Units 

% Factors compute the projection of the input data with a set of filters (columns of matrix VF).
% Their output is squared because each factor is connected twice to the same data
% with the same set of filters. The square outputs are sent to binary
% hidden units after projection with a second layer matrix (matrix FH)
% that pools similar filters. Because the second layer matrix is non-positive the binary hidden units use their “off” states to represent
% abnormalities in the covariance structure of the data.
...
%         epsilonb_meanc = epsilonb_mean/max(1,t/20);
        ...
        for batch=0:num_batches-1
            ...
            % Visible bias
            bias_visinc = sum(minidata,2) .* (-1);

            % Mean part
            feat_mean = sigmoid((W'*minidata) + hb_mean) .* (-1);
            W_meaninc = minidata*feat_mean';
            ...
            if doPCD == 0 %  CD-1 (set negative data to current training samples)
                [hmc_step, hmc_ave_rej,negdata] = draw_HMC_samples(minidata,VF,FH,hb_cov,vb,W,hb_mean,hmc_step,hmc_step_nr,hmc_ave_rej,hmc_target_ave_rej,batch_size,small,num_vis);
